...
export class SqliteInterface {
  ...
  async testConnection(): Promise<string | null> {
    return this.select<string>("SELECT sql FROM sqlite_master WHERE name='bookmarks' AND name='tags';")
  }

  async select<T>(query: string, bindValues?: unknown[]): Promise<T | null> {
    if (!this.#database) {
      return null
    }
    const result = await this.#database.select<T>(query, bindValues)
